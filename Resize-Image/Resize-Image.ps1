...
$files = Get-ChildItem -Path $inputDirs -File | Where-Object {$exts -contains $_.Extension.ToLower()}
...
Function Resize-Image() {
    ...
    Process {
        ForEach ($Image in $ImagePath) {
            ...
            $Bitmap = New-Object -TypeName System.Drawing.Bitmap -ArgumentList $Width, $Height -ErrorAction Stop
...
foreach ($file in $files) {
    $outputPath = $file.DirectoryName + $outputDir
    if (-not (Test-Path $outputPath)) {
        New-Item -Path $outputPath -ItemType Directory
    }
    Resize-Image -ImagePath $file.FullName -Width 1920 -MaintainRatio -NameModifier "resized"
}